...
const PrimaryView = ({ mood, setMood, setWelcome, sectionStyles }: { mood: string, setMood: (mood: string) => void, setWelcome: (welcome: string) => void, sectionStyles: string }) => {
  ...
  return (
      <BrowserRouter>
        <main className='fixed flex flex-col items-center justify-between h-full w-full max-w-screen flex-1 text-white'>
            {/* <CollapsedNavBar setCollapsed={setCollapsed} /> */}
            <NavBar setCollapsed={setCollapsed} audioRef={audioRef} currentTrack={currentTrack} />
            <Routes>
